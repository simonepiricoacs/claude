---
name: karate-testing
description: Comprehensive knowledge base for Water Framework Karate REST API testing, including feature file patterns, karate-config.js setup, test runner configuration, and integration with Water test runtime. Use when implementing, reviewing, or debugging Karate tests for Water REST endpoints.
...
Use one feature file per resource or high-level use case:

| Pattern | When to use | Example |
|---------|-------------|---------|
| `<entity>-crud.feature` | Basic CRUD + pagination for an entity | `book-crud.feature` |
| `<entity>-<usecase>.feature` | State transitions and custom actions | `book-lost.feature`, `loan-close.feature` |
| `security.feature` | Role-based access denial cases | `security.feature` |

**Required `it.water.application.properties` for every test module** (without these, tests will fail):
...
- Without `water.testMode=true`: the JWT filter rejects every request with 401.
- Without `water.rest.security.jwt.validate=false`: tests fail at token signature validation even with impersonation.
...
### ❌ Mistake 5: Incorrect EntityVersion in Update
...
1. **One Scenario Per Workflow**: Group related CRUD operations (CREATE → UPDATE → FIND → DELETE) in a single scenario for proper cleanup.
2. **Always set Content-Type and Accept headers**: Prevents 406/415 errors.
3. **Use fuzzy matching for timestamps and IDs**: `'#number'`, `'#string'` — avoids brittle tests.
4. **Test both success and error cases**: 200, 400, 404, 409, 422.
5. **Use descriptive scenario names**: `Publish without channel should fail` not `Test 2`.
6. **Capture variables for dependent steps**: `* def entityId = response.id`.
7. **Comment test sections**: Use `# ==================== CREATE ====================`.
8. **Use Scenario Outline for validation tests**: Parameterize boundary value tests.
...
- Ensure all `@Inject` fields have `@Setter` (Lombok) for injection
- Verify `@FrameworkComponent` implementations are on test classpath

### Issue: 401 Unauthorized (Unexpected)
...
### Issue: 422 Validation Error (Unexpected)
- Check entity class `@NotNull`, `@NotBlank` constraints
- Ensure request JSON includes all required fields with valid values

### Issue: Variables Not Recognized
- Ensure `karate-config.js` is in `src/test/resources/`
- Verify the function returns an object with the `serviceBaseUrl` key
- Check test runner passes `webServerPort`, `host`, `protocol` system properties
...
```gherkin
Then match response == { id: '#number' }        # type
Then match response contains { name: 'Admin' }  # partial
Then match response.results == '#array'          # array
Then assert response.results.length >= 2         # assertion
```
...
function fn() {
    ...
    let protocol = karate.properties['protocol'];
    let serviceBaseUrl = protocol+"://"+host+":"+webServerPort;
    return { "serviceBaseUrl": serviceBaseUrl }
}
```